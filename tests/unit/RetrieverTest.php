 ...
 *
 * Fahad_AI_Retriever runs the keyword leg (WooCommerce search) and the vector leg
 * (embed query -> vector store) and fuses them with RRF. It plugs into the
 * EXISTING `fahad_ai_semantic_retriever` seam (from #60), so search_products
...
class RetrieverTest extends TestCase {
	...
	public function test_search_fuses_keyword_and_vector_with_rrf(): void {
		...
		$ids = ( new Fahad_AI_Retriever( $this->provider(), $store ) )->search( 'warm', [], 10 );
	...
	public function test_search_respects_k(): void {
		...
		$this->assertCount( 2, ( new Fahad_AI_Retriever( $this->provider(), $store ) )->search( 'warm', [], 2 ) );
	}

	public function test_search_returns_empty_when_query_cannot_be_embedded(): void {
		$store = Mockery::mock( Fahad_AI_Vector_Store::class );
		$store->shouldNotReceive( 'query' );
		$retriever = new Fahad_AI_Retriever( $this->provider( [] ), $store ); // embed returns nothing
	...
	public function test_search_returns_empty_when_index_has_no_match_so_keyword_takes_over(): void {
		...
		$this->assertSame( [], ( new Fahad_AI_Retriever( $this->provider(), $store ) )->search( 'warm', [], 10 ) );
	}
	...
	public function test_seam_callback_passes_through_when_no_provider(): void {
		...
		$this->assertNull( Fahad_AI_Retriever::resolve_seam( null, 'warm', [] ) );
	}
}
